...
export default class ProductFilter extends LightningElement {
    searchKey = '';
    maxPrice = 10000;
    pageSize = 9; // Task 2 Page Size Filter
    ...
    /**
     * Task 2 Page Size Filter
     * Event Handler for change item size per page
     */
    ...
    handleCheckboxChange(event) {
        if (!this.filters.categories) {
            ...
            // Frame Color Filter
            this.filters.frameColor = this.frameColor.data.values.map(
                (item) => item.value
            );
        }
