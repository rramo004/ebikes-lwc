...
export default class ProductCard extends NavigationMixin(LightningElement) {
    ...
    //Sections which are active
    section = ['EC', 'Frame', 'Brakes'];
    ...
    handleRecordLoaded(event) {
        ...
        this.pname = getFieldValue(recordData, NAME_FIELD);
        this.pname = this.pname.replace(/ /g,"+");

        // Sends an HTTP GET REQUEST and retrieves the json response
        fetch('https://vpsdevds2-viasat.cs32.force.com/paymentservice/services/apexrest/ebikestockcheck/?pname=' + this.pname, // End point URL
        {
            // Request type
            method:"GET",
            headers:{
        ...
        .then((jsonResponse) => {
            this.nextRestockDate = jsonResponse['nextRestockDate'];
            this.currentInventory = jsonResponse['currentInventory'];
        })
        .catch(error => {
            console.log('Error with API Call: '+JSON.stringify(error));
        })
    }
     ...
     * Event Handler for the accordion header events
     */
